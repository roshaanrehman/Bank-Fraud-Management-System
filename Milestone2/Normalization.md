...
* Primary key consists of a single attribute (User_ID).
* All non-key attributes fully depend on User_ID.

### 3NF ✅

* Role does not determine Status.
* Username is an alternate key but does not create transitive dependency.
* All non-key attributes depend only on User_ID.

**Justification:** USERS is in 3NF. It stores authentication and authorization information without redundancy.
...
* All values are atomic.
* Audit_ID uniquely identifies each log record.
* No repeating groups exist.

### 2NF ✅

* Single-column primary key.
* Every non-key attribute depends on Audit_ID.

### 3NF ✅

* Action does not determine Module.
* Module does not determine Details.
* User information is referenced using a foreign key rather than duplicated.

**Justification:** AUDIT_LOG is in 3NF. It tracks system activities while avoiding redundancy.
...
| Case_ID         | INT      | FK → INVESTIGATION_CASE |
| Investigator_ID | INT      | FK → INVESTIGATOR       |
...
* All attributes contain atomic values.
* Note_ID uniquely identifies each note.

### 2NF ✅

* Single-column primary key.
* All attributes fully depend on Note_ID.

### 3NF ✅

* Investigator details are not stored directly.
* Case details are not duplicated.
* All non-key attributes depend only on Note_ID.

**Justification:** CASE_NOTE is in 3NF. It supports investigation tracking while maintaining normalization.

### Updated Conclusion

All tables in the final Bank Fraud Detection System satisfy First Normal Form (1NF), Second Normal Form (2NF), and Third Normal Form (3NF). Data redundancy has been minimized through the use of foreign keys, and each table represents a single entity. The final schema supports fraud detection, investigation management, reporting, auditing, and user authentication while maintaining data integrity and normalization principles.
